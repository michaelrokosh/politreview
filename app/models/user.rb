class User < ActiveRecord::Base
  ...
  def self.find_for_vkontakte_oauth access_token
    if user = User.where(url: access_token.info.urls.Vkontakte).first
      ...
      User.create!(provider: access_token.provider, avatar_url: access_token.info.image, username: access_token.info.name, email: access_token.extra.raw_info.screen_name + '@vk.com', password: Devise.friendly_token[0,20]) 
    end
  end
